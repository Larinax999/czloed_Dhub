...
def setup(proxy_list,hc_a):
    global pl,cookies
    pl = proxy_list
    cookies["hc_accessibility"] = hc_a

def bypass():
    global host,sitekey,cookies,headers
    while True:
        r = None
        p = None
        cap = None
        ...
        while True:
            ...
            try:
                with Client(transport=SyncProxyTransport.from_url(f'socks4://{p}')) as client:
                    a = client.get(f"https://hcaptcha.com/checksiteconfig?host={host}&sitekey={sitekey}&sc=1&swa=1", headers=headers, timeout=5).json()
                if a["pass"]:
                    cap = a.get("generated_pass_UUID")
                    r = a["c"]
                    r["type"] = "hsl"
                else:
                    raise
        ...
        while True:
            try:
                with Client(transport=SyncProxyTransport.from_url(f'socks4://{p}')) as client:
                    a = client.post(f"https://hcaptcha.com/getcaptcha?s={sitekey}",cookies=cookies, data=data, headers=headers, timeout=5).json()
                cap = a['generated_pass_UUID']
            except :
                p = choice(pl).replace('\n','')
            else:
                break
