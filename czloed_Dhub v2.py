...
def empty():
    bal = post('https://api.capmonster.cloud/getBalance',json={"clientKey": config['cap_key']}).json()['balance']
    if bal < 0 :
        return True
    return False
...
# create zone
def token_gen(invite=config["invite_link"], verify_mail=False, verify_phone=False, file_name="token"):
    password = "czloed_Dhub@"
    register_payload = {"fingerprint":"","email": randomstr(randint(3, 6)) + randomint(randint(3, 5)) + "@gmail.com","username":config['username'],"password": password,"invite":invite,"consent":True,"date_of_birth":"1999-03-05","gift_code_sku_id":"","captcha_key":""}
...
def livesteam(token, chid, guild_id):
    ws = WebSocket()
...
def online(token):
    ...
    hello = loads(ws.recv())
    heartbeat_interval = hello['d']['heartbeat_interval']
    ws.send(dumps({"op": 2,"d": {"token": token,"properties": {"$os": "windows","$browser": "Discord","$device": "desktop"},"presence": {"game": {"name": config['game_name'],"type": randint(0,3)},"status": choice(['online', 'dnd', 'idle']),"since": 0,"afk": False}}}))

#executor.submit(get_captcha_code, True)
if empty():
    exit('capmonster balance empty')

def start():
    token = token_gen(verify_mail=False, verify_phone=False, file_name=config['file_name_asve'])
    online(token)
